import { HoverCallback, MATCH_CLASS, NO_MATCH_CLASS
} from '../select/selectcommon';
...
const nodeComparisonTemplate = document.querySelector(".analysis--track-mutations") as HTMLDivElement;
const nodeComparisonContainer = nodeComparisonTemplate?.parentNode as HTMLDivElement;
const mutationTemplate = nodeComparisonTemplate?.querySelector(".analysis--node-comparison--mutation") as HTMLDivElement;
if (!nodeComparisonTemplate || !nodeComparisonContainer || !mutationTemplate) {
  throw new Error("could not find a div to use as a template for node comparisons on the analysis tab");
}
...
const mutationChartSelector = '.series-group-container',
  mutationNameSelector = '.analysis--node-comparison--mutation-name',
  mutationPrevalenceSelector = '.analysis--node-comparison--mutation-prevalence span',
  mutationContainerSelector = '.analysis--mutation-timeline',
  ancestorNodeNameSelector = '.analysis--mutations-ancestor',
  descendantNodeNameSelector = '.analysis--mutations-descendant',
...
export class NodePairMutationList {
  ...
  constructor(data : NodeMutationsData, nodeHighlightCallback: HoverCallback) {
    this.data = data;
    ...
    const mutationContainer = this.div.querySelector(mutationContainerSelector) as HTMLDivElement,
      ...
      // overlapSpan = this.div.querySelector(".analysis--node-overlap-item") as HTMLSpanElement;
  ...
  setMutations():void {
    ...
    this.mutationTimelines = mutationTimelineData.map((md:MutationTimelineData)=>{
      ...
      const mt = new MutationTimeline(md, minDate, maxDate, seriesCallback);
...
export class NodeMutations {
  ...
  setData(nodeComparisonData: NodeMutationsData[],
    ...
    sorted.filter(pair=>pair.mutationCount > 0).forEach(chartData=>{
      ...
      const nc = new NodePairMutationList(chartData, this.nodeHighlightCallback);
      nc.requestDraw();
      this.charts.push(nc);
    });
    return this.charts;
