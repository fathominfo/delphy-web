...
import {addMutationTime, MutationDistribution} from './mutationdistribution';
...
export type pythyaReadyCallbackType = (_:Omphalos)=>void;

export type returnless = ()=>void;
      ...
      const treeCount = tree.getNumBaseTrees(),
        ...
        tallyMutation = (mut:Mutation)=>{
          ...
          if (mutationLookup[name] === undefined) {
            ...
            const md : MutationDistribution = {
              mutation: mut,
              times: [],
              possibleTrees: treeCount,
              isApobecCtx: isApobecCtx
            }
            mutationLookup[name] = md; //new MutationDistribution(mut, treeCount, isApobecCtx);
          }
          addMutationTime(mutationLookup[name], mut.time);
        },
        /*
...
let readyCallback: pythyaReadyCallbackType;


export function setPythiaReadyCallback(fnc: pythyaReadyCallbackType):void { // eslint-disable-line no-unused-vars
  readyCallback = fnc;
  if (globalDelphy) {
    readyCallback(new Omphalos());
  }
}
